...
export class StreamPdfRendererService {
  ...
  renderCoverPage(
    ...
    // 3b. Vector Activity Distribution Mini-Chart
    const chartY = guideY + 148;
    doc.roundedRect(MARGIN_LEFT, chartY, contentWidth, 54, 6).fillAndStroke('#FFFFFF', '#E2E8F0');
    doc.fillColor('#64748B').font(fonts.bold).fontSize(7).text('24-HOUR HOURLY DIURNAL VELOCITY DISTRIBUTION', MARGIN_LEFT + 12, chartY + 8);

    // Render 24 hourly vector bars
    const barWidth = (contentWidth - 28) / 24;
    for (let h = 0; h < 24; h++) {
      const barH = Math.max(3, Math.min(26, Math.sin(h / 3.8) * 14 + (h > 9 && h < 22 ? 16 : 4)));
      const bx = MARGIN_LEFT + 14 + h * barWidth;
      const by = chartY + 44 - barH;
      doc.roundedRect(bx, by, Math.max(2, barWidth - 3), barH, 1.5).fill(h === summaryMetrics.peakHour ? '#0284C7' : '#38BDF8');
    }
